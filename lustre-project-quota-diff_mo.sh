...
/^\+[^\+].*limits:/ {
    ...
    if (hard > 0) {
        ...
        if (pct_delta > 5 || before > hard || after > hard) {
            hard_mo   = hard   / 1048576;
            before_mo = before / 1048576;
            after_mo  = after  / 1048576;
            delta_mo  = delta  / 1048576;
            ...
            printf "%s|%s|%.0f|%.0f|%.0f|%.1f|%.1f|%+.0f|%.1f\n",
                statut, id, hard_mo, before_mo, after_mo,
                pct_used_before, pct_used_after, delta_mo, pct_delta;
        }
    }
}
' | sort -t'|' -k9 -rn | awk -F'|' '
